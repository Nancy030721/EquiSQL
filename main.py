...
def encode_query(ast, idx, variables):
    cond_join = encode_join_clause(ast, idx, variables)
...
# todo: some bug here
def encode_join_clause(ast, idx, variables):
    ...
    left_table = str(ast.args.get("from").args["this"].name)
    encoding = True
    ...
    for i in range(len(joins)) :
        ...
        right_table = str(join.args["this"].name)

        # print(f"line228, left_table = {left_table}, right_table = {right_table}")
        if (not join.side): # inner join
            ...
            left_row = variables["row_identity"][left_table]
            right_row = variables["row_identity"][right_table]  
